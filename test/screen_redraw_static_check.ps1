...
$requiredDisplayApis = @(
  ...
  'drawVoltageLockoutFrame',
  'updateVoltageLockoutValues'
)
...
$requiredControllerCalls = @(
  ...
  'display_.drawVoltageLockoutFrame',
  'display_.updateVoltageLockoutValues'
)
...
$lockoutUpdateStart = $displaySource.IndexOf('void DisplayManager::updateVoltageLockoutValues')
$hardwareFaultStart = $displaySource.IndexOf('void DisplayManager::drawHardwareFault')
if ($lockoutUpdateStart -lt 0 -or $hardwareFaultStart -lt $lockoutUpdateStart) {
  throw 'Could not locate updateVoltageLockoutValues body'
}

$lockoutUpdateBody = $displaySource.Substring($lockoutUpdateStart, $hardwareFaultStart - $lockoutUpdateStart)
if ($lockoutUpdateBody -match 'fillScreen') {
  throw 'Lockout incremental update must not clear the screen every refresh'
}

if ($displayHeader -notmatch 'lockoutValuesPrimed_') {
  throw 'DisplayManager must cache lockout values for incremental redraw'
}
